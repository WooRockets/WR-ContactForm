...
(function($) {
	$.WR_CF_ProductAddons = function(params) {
		// Object parameters
		this.params = $.extend({
			base_url: '',
			core_plugin: '',
		...
		// Initialize WooRockets addons management
		$(document).ready($.proxy(this.init, this));
	};

	$.WR_CF_ProductAddons.prototype = {
		init: function() {
			var self = this;
		...
		request: function(action, identification, authentication, secondary_action) {
			...
			if (action == 'authenticate' && !self.params.has_saved_account) {
				self.authentication.find('.btn-primary').addClass('wr-loading disabled').attr('disabled', 'disabled').html(self.lang.AUTHENTICATING);
			} else {
				// Toggle status
				switch (action) {
					case 'authenticate':
				...
				addon.addClass('wr-loading').parent().children('a[data-action]').addClass('disabled').attr('disabled', 'disabled');
			}
			...
			$.ajax({
				...
				if (data.success) {
					...
					if (action == 'authenticate') {
						...
						// Clear status
						addon.removeClass('wr-loading');

						// Execute secondary action
						self.request(secondary_action, identification, authentication);
					} else {
						// Toggle status
						addon.removeClass('wr-loading').parent().children('a[data-action]').removeClass('disabled').removeAttr('disabled');
					...
					if (action == 'authenticate') {
						...
						self.authentication.find('.btn-primary').removeClass('wr-loading').html(self.lang.INSTALL);
					} else {
