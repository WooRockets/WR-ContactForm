...
class WR_CF_Form {
	 ...
	 * @var  string
	 */
	protected $text_domain = WR_CF_LIBRARY_TEXTDOMAIN;

	/**
	 * Instantiate a WR_CF_Form object then return.
	 *
	...
	public static function get_instance( $form_id, $fields = array() ) {
		if ( ! isset( $instances[ $form_id ] ) ) {
			$instances[ $form_id ] = new WR_CF_Form( $form_id, $fields );
		}
	...
	public function setup_form( $fields, $override = false ) {
		// Filter form fields
		$fields = apply_filters( 'wr_cf_setup_form', $fields );
	...
	protected function add_field( $field_id, $field_attrs = null, $section_id = 'default', $fieldset_id = null, $tab_id = null, $accordion_id = null ) {
		if ( is_array( $field_id ) ) {
			...
			if ( ! isset( $field_attrs['text_domain'] ) && WR_CF_LIBRARY_TEXTDOMAIN != $this->text_domain ) {
			...
			$field = WR_CF_Form_Field::get_instance( $field_attrs );
