...
<div class="wrap jsn-bootstrap3">
	<h2><?php _e( $plugin['Name'], WR_CF_LIBRARY_TEXTDOMAIN ); ?> <?php _e( 'Add-ons', WR_CF_LIBRARY_TEXTDOMAIN ); ?></h2>
	<p>
		<?php printf( __( 'Extend %s functionality with following add-ons', WR_CF_LIBRARY_TEXTDOMAIN ), __( $plugin['Name'], WR_CF_LIBRARY_TEXTDOMAIN ) ); ?>
	</p>
		...
		<ul id="<?php echo '' . $plugin['Identified_Name']; ?>-addons" class="thumbnails clearfix">
			...
			<li class="thumbnail pull-left">
				<a href="<?php echo esc_url( $details->url ); ?>" target="_blank">
					<img src="<?php echo esc_url( $details->thumbnail ); ?>" alt="<?php esc_attr_e( $details->name, WR_CF_LIBRARY_TEXTDOMAIN ) ?>" />
				</a>
				<?php if ( ! $details->compatible ) : ?>
				<span class="label label-danger"><?php _e( 'Incompatible', WR_CF_LIBRARY_TEXTDOMAIN ); ?></span>
				<?php elseif ( $details->installed ) : ?>
				<span class="label label-success"><?php _e( 'Installed', WR_CF_LIBRARY_TEXTDOMAIN ); ?></span>
				<?php endif; ?>
				<div class="caption">
					<h3><?php _e( $details->name, WR_CF_LIBRARY_TEXTDOMAIN ) ?></h3>
					<p><?php _e( $details->description, WR_CF_LIBRARY_TEXTDOMAIN ) ?></p>
					<div class="actions clearfix">
						<div class="pull-left">
							...
							<a class="btn btn-primary <?php if ( ! $details->compatible ) echo 'disabled'; ?>" href="javascript:void(0);" <?php if ( $details->compatible ) : ?>data-action="install" data-authentication="<?php echo absint( $details->authentication ); ?>" data-identification="<?php echo '' . $details->identified_name; ?>"<?php endif; ?>>
								<?php _e( 'Install', WR_CF_LIBRARY_TEXTDOMAIN ); ?>
							</a>
							...
							<a class="btn btn-primary <?php if ( ! $details->compatible ) echo 'disabled'; ?>" href="javascript:void(0);" data-action="update" <?php if ( $details->compatible ) : ?>data-authentication="<?php echo absint( $details->authentication ); ?>" data-identification="<?php echo '' . $details->identified_name; ?>"<?php endif; ?>>
								<?php _e( 'Update', WR_CF_LIBRARY_TEXTDOMAIN ); ?>
							</a>
							<?php endif; ?>
							<a class="btn <?php if ( ! $details->updatable ) echo 'btn-primary'; ?> <?php if ( ! $details->compatible ) echo 'incompatible'; ?>" href="javascript:void(0);" data-action="uninstall" data-authentication="<?php echo absint( $details->authentication ); ?>" data-identification="<?php echo '' . $details->identified_name; ?>">
								<?php _e( 'Uninstall', WR_CF_LIBRARY_TEXTDOMAIN ); ?>
							</a>
							<?php endif; ?>
						</div>
						<a class="btn btn-info pull-right" href="<?php echo esc_url( $details->url ); ?>" target="_blank">
							<?php _e( 'More Info', WR_CF_LIBRARY_TEXTDOMAIN ); ?>
						</a>
					</div>
				</div>
			</li>
			...
			<div class="modal-content">
				<div class="modal-header">
					...
					<h4 class="modal-title" id="<?php echo '' . $plugin['Identified_Name']; ?>-authentication-modal-label">
						<?php _e( 'WooRockets Customer Account', WR_CF_LIBRARY_TEXTDOMAIN ); ?>
					</h4>
				</div>
				<div class="modal-body">
					<form name="WR_Addons_Authentication" method="POST" class="form-horizontal" autocomplete="off">
						...
						<div class="form-group clearfix">
							<label class="col-sm-3 control-label" for="username"><?php _e( 'Username', WR_CF_LIBRARY_TEXTDOMAIN ); ?>:</label>
							<div class="col-sm-9">
								<input type="text" value="" class="form-control" id="username" name="username" autocomplete="off" />
							</div>
						</div>
						<div class="form-group clearfix">
							<label class="col-sm-3 control-label" for="password"><?php _e( 'Password', WR_CF_LIBRARY_TEXTDOMAIN ); ?>:</label>
							<div class="col-sm-9">
							...
							<div class="col-sm-9 pull-right">
								<div class="checkbox-inline">
									<label>
										...
										<?php _e( 'Remember Me', WR_CF_LIBRARY_TEXTDOMAIN ); ?>
									</label>
								</div>
							</div>
						</div>
				...
				<div class="modal-footer">
					<button type="button" class="btn btn-primary"><?php _e( 'Install', WR_CF_LIBRARY_TEXTDOMAIN ); ?></button>
					<button type="button" class="btn btn-default" data-dismiss="modal"><?php _e( 'Cancel', WR_CF_LIBRARY_TEXTDOMAIN ); ?></button>
				</div>
			</div>
		</div>
	</div>
...
$script = '
		new $.WR_CF_ProductAddons({
			...
			language: {
				CANCEL: "' . __( 'Cancel', WR_CF_LIBRARY_TEXTDOMAIN ) . '",
				INSTALL: "' . __( 'Install', WR_CF_LIBRARY_TEXTDOMAIN ) . '",
				UNINSTALL: "' . __( 'Uninstall', WR_CF_LIBRARY_TEXTDOMAIN ) . '",
				INSTALLED: "' . __( 'Installed', WR_CF_LIBRARY_TEXTDOMAIN ) . '",
				INCOMPATIBLE: "' . __( 'Incompatible', WR_CF_LIBRARY_TEXTDOMAIN ) . '",
				UNINSTALL_CONFIRM: "' . __( 'Are you sure you want to uninstall %s?', WR_CF_LIBRARY_TEXTDOMAIN ) . '",
				AUTHENTICATING: "' . __( 'Verifying...', WR_CF_LIBRARY_TEXTDOMAIN ) . '",
				INSTALLING: "' . __( 'Installing...', WR_CF_LIBRARY_TEXTDOMAIN ) . '",
				UPDATING: "' . __( 'Updating...', WR_CF_LIBRARY_TEXTDOMAIN ) . '",
				UNINSTALLING: "' . __( 'Uninstalling...', WR_CF_LIBRARY_TEXTDOMAIN ) . '",
			}
		});';
