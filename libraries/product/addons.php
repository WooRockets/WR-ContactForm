...
if ( ! class_exists( 'WR_CF_Product_Addons' ) ) :
 ...
 * @since    1.0.0
 */
class WR_CF_Product_Addons extends WR_CF_Product_Info {
	/**
	...
	public static function manage() {
		...
		try {
			...
			if ( empty( $addon ) ) {
				throw new Exception( __( 'Missing addon to be installed.', WR_CF_LIBRARY_TEXTDOMAIN ) );
			}
			...
			if ( empty( $action ) || ! array_key_exists( $action, self::$actions ) ) {
				throw new Exception( __( 'The requested action is invalid.', WR_CF_LIBRARY_TEXTDOMAIN ) );
			}
	...
	protected static function execute( $addon, $action ) {
		...
		if ( 'update' == $action || 'uninstall' == $action ) {
			...
			if ( empty( $plugin ) ) {
				throw new Exception(
					__( 'update' == $action ? 'Cannot detect plugin to be updated.' : 'Cannot detect plugin to be removed.', WR_CF_LIBRARY_TEXTDOMAIN )
				);
			}
		}
		...
		if ( 'uninstall' == $action ) {
			...
			$wp_filesystem = WR_CF_Init_File_System::get_instance();
			...
			if ( 'authenticate' != $action ) {
				...
				if ( isset( $_GET['core'] ) && ( $core = self::get( $_GET['core'] ) ) ) {
					...
					if ( $core_version && version_compare( $core_version, $core['Version'], '>' ) ) {
						...
						throw new Exception(
							sprintf(
								__( "Cannot install %1\$s v%2\$s.\nThis version requires %3\$s v%4\$s while you are using %5\$s v%6\$s.", WR_CF_LIBRARY_TEXTDOMAIN ),
				...
				if ( isset( $_POST['remember'] ) && (boolean) $_POST['remember'] ) {
					update_option( 'wr_cf_customer_account', array( 'username' => $username, 'password' => $password ) );
				}
			}
		}
	...
	protected static function download( $addon, $authentication = false, $username = null, $password = null, $authenticate_only = false ) {
		...
		if ( $wp_filesystem->size( $result ) < 10 ) {
			...
			switch ( $content ) {
				case 'ERR00':
					throw new Exception( __( 'Invalid Parameters! Can not verify your product information.', WR_CF_LIBRARY_TEXTDOMAIN ) );
				break;

				case 'ERR01':
					throw new Exception( __( 'Invalid username or password.', WR_CF_LIBRARY_TEXTDOMAIN ) );
				break;

				case 'ERR02':
					throw new Exception( __( 'We could not find the product in your order list. Seems like you did not purchase it yet.', WR_CF_LIBRARY_TEXTDOMAIN ) );
				break;

				case 'ERR03':
					throw new Exception( __( 'Requested file is not found on server.', WR_CF_LIBRARY_TEXTDOMAIN ) );
				break;

				default:
					if ( 'true' != $content && 'yes' != $content && '1' != $content ) {
						throw new Exception( __( 'Cannot connect to WooRockets.com server.', WR_CF_LIBRARY_TEXTDOMAIN ) );
					}
				break;
			}
		}
		...
		if ( ! $authenticate_only ) {
			if ( ! $wp_filesystem->copy( $result, $target, true, FS_CHMOD_FILE ) ) {
				...
				if ( ! $wp_filesystem->copy( $result, $target, true, FS_CHMOD_FILE ) ) {
					throw new Exception( __( 'Cannot store add-on package to local file system.', WR_CF_LIBRARY_TEXTDOMAIN ) );
				}
			}
		}
	...
	protected static function activate( $addon, $action = '' ) {
		...
		if ( empty( $plugin ) ) {
			throw new Exception( __( 'Cannot detect plugin to activate.', WR_CF_LIBRARY_TEXTDOMAIN ) );
		}
